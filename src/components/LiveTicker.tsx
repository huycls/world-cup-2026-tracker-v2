...
export const LiveTicker: React.FC<LiveTickerProps> = ({ matches, teams }) => {
  ...
  const getTeamDisplay = (match: Match, slot: 1 | 2) => {
    ...
    const displayName = team?.name ?? name ?? "Chờ xác định";
    const displayCode = team?.code ?? code;
    const showFlag =
      !isPlaceholderTeamLabel(displayName) && hasValidFlagCode(displayCode);
    return {
      name: displayName,
      code: showFlag ? displayCode! : null,
    };
  };
      ...
      <div className="flex-1 flex flex-col min-h-[180px] overflow-hidden">
                    ...
                    <div className="grid grid-cols-7 items-center gap-1">
                      <div className="col-span-2 flex flex-col items-center">
                        {t1.code ? (
                          ...
                          <div className="w-8 h-5 rounded bg-neutral-800 border border-neutral-700 flex items-center justify-center text-[10px] text-neutral-500 font-bold">
                            ?
                          </div>
                        )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-h-[140px] border-t border-[#D4AF37]/20 pt-3">
          <h4 className="text-xs font-sans font-bold text-neutral-400 uppercase tracking-wider flex items-center gap-1.5 mb-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-[#D4AF37]" /> Kết Quả Gần
            Đây
          </h4>
              ...
              recentFinished.map((m) => {
                ...
                return (
                  <div
                    ...
                    <div className="font-bold text-neutral-200 flex justify-between relative">
                      {t1.name}{" "}
                      <span className="w-fit text-yellow-500 absolute left-0 right-0 mx-auto">
                        {m.score1} - {m.score2}
                      </span>{" "}
                      {t2.name}
                    </div>
